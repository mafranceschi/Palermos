...
export default async function DashboardPage({
  ...
  return (
    <div className="space-y-6">
      ...
      <form className="grid grid-cols-2 gap-3 rounded-lg border border-neutral-200 bg-white p-4 sm:grid-cols-4">
        <input
          type="text"
          name="plate"
          defaultValue={plate}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {jobs.map((job, i) => (
            <Link
              key={job.id}
              href={`/trabajo/${job.id}`}
              className="flex gap-3 rounded-lg border border-neutral-200 bg-white p-3 hover:border-neutral-400"
            >
